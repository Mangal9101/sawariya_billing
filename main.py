...
from sqlalchemy import (
    ...
    DateTime, ForeignKey, Text, func, text, event, and_
)
...
# =========================================================
# AUTOMATIC MULTI-USER DATA ISOLATION
# =========================================================

OWNED_MODELS = (Product, Customer, Invoice, InvoiceItem, Purchase, StockMovement, Staff, Khatabook, KhatabookEntry)

@event.listens_for(Session, "do_orm_execute")
def _add_user_filter(execute_state):
    uid = current_user_id()
    if not uid or not execute_state.is_select or execute_state.is_column_load or execute_state.is_relationship_load:
        return
    from sqlalchemy.orm import with_loader_criteria
    for model in OWNED_MODELS:
        execute_state.statement = execute_state.statement.options(
            with_loader_criteria(model, lambda cls: cls.user_id == uid, include_aliases=True, track_closure_variables=True)
        )


@event.listens_for(Session, "before_flush")
def _stamp_owner_id(session, flush_context, instances):
    uid = current_user_id()
    if not uid:
        return
    for obj in session.new:
        if isinstance(obj, OWNED_MODELS) and not getattr(obj, "user_id", None):
            obj.user_id = str(uid)
...
def run_migrations():
    ...
    # Add multi-user ownership columns to older installations.
    owned_tables = ["products", "customers", "invoices", "invoice_items", "purchases", "stock_movements", "staff", "khatabook", "khatabook_entries"]
    try:
        with engine.begin() as connection:
            for table in owned_tables:
                if engine.dialect.name == "postgresql":
                    connection.execute(text(f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS user_id VARCHAR(255)"))
                    ...
                    if "user_id" not in cols:
                        connection.execute(text(f"ALTER TABLE {table} ADD COLUMN user_id VARCHAR(255)"))
    except Exception:
        pass
...
def otp_hash(code: str) -> str:
    secret = os.getenv("SECRET_KEY", "change-this-secret").encode()
    return hmac.new(secret, code.encode(), hashlib.sha256).hexdigest()


def send_otp_email(email: str, name: str, code: str):
    if not SMTP_EMAIL or not SMTP_PASSWORD:
        # Development fallback: never expose OTP in production responses.
        print(f"[OTP] SMTP not configured. OTP for {email}: {code}")
        return False

    msg = MIMEMultipart()
    msg["From"] = SMTP_EMAIL
    msg["To"] = email
    msg["Subject"] = "Sawariya Billing - Login OTP"
    msg.attach(MIMEText(
        f"Hello {name or 'User'},\n\nYour Sawariya Billing login OTP is: {code}\n\nThis OTP is valid for {OTP_TTL_MINUTES} minutes.\nIf you did not request this, ignore this email.",
        "plain"
    ))
...
def google_authorization_url(state: str):
    params = {
...
def google_token_exchange(code: str):
    data = urlencode({
        "code": code,
        "client_id": GOOGLE_CLIENT_ID,
...
def claim_legacy_data(db_session, user_id: int):
    """Assign records created before multi-user mode to the first Google user."""
    uid = str(user_id)
    for model in OWNED_MODELS if "OWNED_MODELS" in globals() else ():
        db_session.query(model).filter(model.user_id.is_(None)).update(
            {model.user_id: uid}, synchronize_session=False
        )


def begin_otp_login(request: Request, user: User, db_session):
    code = issue_otp(user)
    db_session.commit()
    try:
        send_otp_email(user.email, user.name or "User", code)
    except Exception:
        db_session.rollback()
        raise
...
def authenticated_user(request: Request):
    ...
    if not uid or not email:
        return None
    return {"id": str(uid), "email": email, "name": request.session.get("name") or email}
...
def _sync_after_commit(session):
    uid = session.info.pop("sync_user_id", None)
    if not uid or not GOOGLE_SHEETS_ID or not GOOGLE_SERVICE_ACCOUNT_JSON:
        return
    try:
        from google_sheets import sync_user_data
        threading.Thread(target=sync_user_data, args=(str(uid),), daemon=True).start()
    except Exception as exc:
        print(f"[Google Sheets] sync start failed: {exc}")


@event.listens_for(Session, "before_commit")
def _remember_sync_user(session):
    uid = current_user_id()
    if uid and (session.new or session.dirty or session.deleted):
        session.info["sync_user_id"] = str(uid)


def login_required(request: Request):
    user = authenticated_user(request)
    CURRENT_USER_ID.set(str(user["id"]) if user else None)
...
def login_page(request: Request):
    pending = request.session.get("pending_user_id")
...
def google_callback(request: Request, code: str = "", state: str = "", error: str = ""):
    if error:
        return RedirectResponse("/login?error=" + quote(error), status_code=303)
    if not code or not state or state != request.session.get("oauth_state"):
        return RedirectResponse("/login?error=Invalid+Google+login+state", status_code=303)

    try:
        token = google_token_exchange(code)
        info = google_userinfo(token["access_token"])
    except Exception as exc:
        return RedirectResponse("/login?error=" + quote(f"Google login failed: {exc}"), status_code=303)

    email = str(info.get("email") or "").strip().lower()
    sub = str(info.get("sub") or "").strip()
    if not email or not sub or not info.get("email_verified", False):
        return RedirectResponse("/login?error=Google+email+could+not+be+verified", status_code=303)

    d = db()
    try:
        user = d.query(User).filter(func.lower(User.email) == email).first()
        is_new_user = user is None
        if not user:
            user = User(email=email, google_sub=sub, name=info.get("name"), picture=info.get("picture"), email_verified=1)
            d.add(user)
            d.flush()
            # Preserve data from the old single-user installation for the first Google account.
            if d.query(User).count() == 1:
                claim_legacy_data(d, user.id)
        else:
            user.google_sub = sub
        ...
        begin_otp_login(request, user, d)
    except Exception as exc:
        d.rollback()
        return RedirectResponse("/login?error=" + quote(f"Could not send OTP: {exc}"), status_code=303)
    finally:
        d.close()

    request.session["oauth_state"] = None
    return RedirectResponse("/login?otp=1", status_code=303)
...
def resend_otp(request: Request):
    ...
    if not uid:
        return RedirectResponse("/login?error=Please+login+with+Google+first", status_code=303)
    d = db()
    try:
        user = d.get(User, int(uid))
        if not user:
            request.session.clear()
            return RedirectResponse("/login?error=User+not+found", status_code=303)
        begin_otp_login(request, user, d)
        return RedirectResponse("/login?otp=1&sent=1", status_code=303)
    except Exception as exc:
        d.rollback()
        return RedirectResponse("/login?error=" + quote(f"OTP could not be sent: {exc}"), status_code=303)
    finally:
        d.close()
...
def verify_otp(request: Request, otp: str = Form(...)):
    uid = request.session.get("pending_user_id")
    if not uid:
        return RedirectResponse("/login?error=OTP+session+expired", status_code=303)
    code = str(otp).strip()
    d = db()
    try:
        user = d.get(User, int(uid))
        if not user or not user.otp_expires_at or datetime.utcnow() > user.otp_expires_at:
            return RedirectResponse("/login?error=OTP+expired.+Please+login+again", status_code=303)
        if (user.otp_attempts or 0) >= 5:
            return RedirectResponse("/login?error=Too+many+OTP+attempts", status_code=303)
        if not hmac.compare_digest(user.otp_hash or "", otp_hash(code)):
            user.otp_attempts = (user.otp_attempts or 0) + 1
            d.commit()
            return RedirectResponse("/login?otp=1&error=Invalid+OTP", status_code=303)
...
# Legacy username/password login intentionally removed.
@app.post("/login")
def legacy_login_disabled(request: Request):
    return RedirectResponse("/login?error=Please+use+Continue+with+Google", status_code=303)
...
def google_sheets_sync(request: Request):
    user = authenticated_user(request)
    if not user:
        return JSONResponse({"error": "unauthorized"}, status_code=401)
    if not GOOGLE_SHEETS_ID or not GOOGLE_SERVICE_ACCOUNT_JSON:
        return JSONResponse({"error": "Google Sheets is not configured"}, status_code=503)
    try:
        from google_sheets import sync_user_data
        ok = sync_user_data(str(user["id"]))
        return {"ok": bool(ok), "message": "Google Sheets synced"}
    except Exception as exc:
