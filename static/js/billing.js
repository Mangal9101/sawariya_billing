...
function loadDraft() {
    try {
        ...
        if (Array.isArray(draft.cart)) {
            cart = draft.cart.filter(x => x && Number(x.product_id) > 0 && Number(x.quantity) > 0);
        }
        if (draft.customer) {
        ...
        if (discountInput && draft.discount !== undefined) discountInput.value = draft.discount;
        if (paymentInput && draft.payment) paymentInput.value = draft.payment;
        if (paidInput && draft.paid !== undefined) paidInput.value = draft.paid;
        render();
    } catch (error) {
...
function clearDraft() { localStorage.removeItem(BILL_DRAFT_KEY); }
...
if (ps) ps.addEventListener("input", searchProducts);
...
function addProduct(p) {
    ...
    if (x) {
        ...
        cart.push({ product_id: id, name: String(p.name || "Product"), quantity: 1, price: Number(p.wholesale || 0), stock });
    }
    if (ps) ps.value = "";
...
function render() {
    ...
    body.innerHTML = cart.map((x, i) => {
        ...
        return `<tr><td>${esc(x.name)}</td><td><input class="qty" type="number" min="1" max="${Number(x.stock) || 1}" value="${qty}" onchange="setQty(${i}, this.value)"></td><td>₹${price.toFixed(2)}</td><td>₹${(price * qty).toFixed(2)}</td><td class="delete-cell"><button type="button" class="delete-btn" onclick="removeItem(${i})">✕</button></td></tr>`;
    }).join("");
    calc();
}
...
function setQty(i, value) {
    ...
    if (stock <= 0) cart.splice(i, 1);
    else cart[i].quantity = Math.min(q, stock);
    render();
    saveDraft();
}

function removeItem(i) { cart.splice(i, 1); render(); saveDraft(); }

function calc() {
    const sub = cart.reduce((sum, x) => sum + (Number(x.price) || 0) * (Number(x.quantity) || 0), 0);
...
if (discountInput) discountInput.addEventListener("input", () => { calc(); saveDraft(); });
[customerName, customerPhone, customerAddress].forEach(el => { if (el) el.addEventListener("input", saveDraft); });
if (paymentInput) paymentInput.addEventListener("change", saveDraft);
if (paidInput) paidInput.addEventListener("input", saveDraft);

async function saveBill() {
    if (!cart.length) { alert("Add at least one product."); return; }
    const total = calc();
    let paid = Math.max(0, parseFloat(paidInput?.value) || 0);
    if (paid > total) { paid = total; if (paidInput) paidInput.value = paid.toFixed(2); }

    const payload = {
        customer: { name: customerName?.value.trim() || "", phone: customerPhone?.value.trim() || "", address: customerAddress?.value.trim() || "" },
        items: cart.map(x => ({ product_id: Number(x.product_id), quantity: Number(x.quantity) })),
    ...
    try {
        const r = await fetch("/billing/save", { method: "POST", headers: { "Content-Type": "application/json", "Accept": "application/json" }, body: JSON.stringify(payload) });
        let data = {};
        ...
        clearDraft();
        if (data.invoice_id) { window.location.href = `/invoice/${data.invoice_id}`; return; }
        alert(`Bill ${data.invoice_no || ""} saved.\n\nTotal ₹${Number(data.total || 0).toFixed(2)}\nDue ₹${Number(data.due || 0).toFixed(2)}`);
        cart = [];
        render();
    } catch (error) {
